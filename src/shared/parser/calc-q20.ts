...
export function propAt20Quality(
  ...
  // EE2 projects armour/evasion/ES totals as if the item had at least 20%
  // quality (a common manual trading heuristic - quality is cheap to add,
  // so buyers compare "as if Q20'd"). That surprises a default that's
  // otherwise meant to match the item's actual displayed values exactly, so
  // this uses the item's real quality instead of a forced 20% floor.
