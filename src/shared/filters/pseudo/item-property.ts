...
import { stat, StatBetter } from "../../data";
...
function armourProps(ctx: FiltersCreationContext) {
  ...
  // EE2 strips the individual AR/EV/ES-contributing mods here since the
  // property total above already reflects them, avoiding a redundant
  // filter. This app shows both: the property total as a distinct "base
  // stats" section, and the individual prefix/suffix roll below it - so the
  // underlying mods should stay visible rather than being removed.
}
...
function weaponProps(ctx: FiltersCreationContext) {
  ...
  // See the matching comment in armourProps() - individual weapon damage
  // mods stay visible instead of being removed in favor of the DPS/physical
  // damage property totals above.
}
