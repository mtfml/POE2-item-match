...
const PSEUDO_RULES: PseudoRule[] = [
  ...
  // },
  // "#% total increased maximum Energy Shield" and "+# total maximum
  // Energy Shield" pseudo-wrapped a single raw stat 1:1 (no combining with
  // other sources like the Life/Mana pseudos do), so they just replaced the
  // mod's own text with a generic pseudo label for no benefit - removed.
  // The armour-property section already shows the item's real total ES.
  // {
...
export function filterPseudo(ctx: FiltersCreationContext) {
  ...
  // EE2 removes any raw stat referenced by a pseudo rule here (resistances,
  // attributes, life/mana) so only the computed pseudo total remains -
  // matching the same pattern already skipped in armourProps/weaponProps
  // above. Kept here too: raw mods (e.g. "39% to Fire Resistance") stay
  // visible in their own right alongside the pseudo total used for search.

  // EE2 collapses per-element resistance totals down to just the single
  // highest one (kept but hidden behind an "advanced" toggle this app
  // doesn't have), dropping the rest - which reads as inconsistent without
  // that toggle (e.g. an item with Cold+Lightning res would show "Cold" and
  // "Total Elemental Resistance" but not Lightning). Skip that collapsing
  // here so every element the item actually has stays visible alongside the
  // combined total.
