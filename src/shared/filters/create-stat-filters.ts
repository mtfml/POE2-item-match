...
import { StatBetter, CLIENT_STRINGS } from "../data";
...
export function initUiModFilters(
  ...
  ctx.filters.push(
    ...ctx.statsByType.map((mod) =>
      // Back to EE2's default (unchecked unless a rule below enables it) -
      // checking every raw mod by default over-constrains the search for
      // items with several variable stats (esp. uniques), multiplying
      // together many ±10% bands and easily finding nothing.
      calculatedStatToFilter(mod, ctx.searchInRange, item),
    ),
  );
...
export function calculatedStatToFilter(
  ...
  if (roll && (!filter.option || roll.option !== roll.value)) {
    ...
    // EE2 forces an exact-value (0%) bracket in several special cases
    // (unmodifiable magic items, perfect rolls, charm slots, etc). Skipped
    // here per explicit request: every stat uses the same percent bracket,
    // no per-stat special-casing.
    //
    // Small flat values (+1 to +N stats like gem/skill levels) get an exact
    // match instead of the percent bracket - a percentage of a small integer
    // rounds to ~0, so the "bracket" ends up being a 2x+ relative jump
    // (e.g. +1 vs +2) rather than a meaningful tolerance.
    if (Math.abs(roll.value) <= 5) {
      percent = 0;
    }
    ...
    // EE2 clamps the default range to the stat's own naturally-possible
    // range (e.g. a mod's tier bounds, `filterBounds` below). Skipped here
    // per explicit request - every stat always gets the full, uncapped
    // ±percent bracket around its actual value, so results stay consistent
    // across stat types instead of being clamped tighter for some (property
    // totals, near-max rolls) than others.
...
function filterFillMinMax(
  ...
  // EE2 only bounds the "better" side by default (e.g. min-only for
  // PositiveRoll stats like Energy Shield) and leaves the other side open,
  // since it's built for "find me an upgrade" shopping - any better item is
  // still a match. This tool is for price-checking comparable recent sales
  // against your own item, so both sides are always bounded by default;
  // widen either side manually in the UI if you want a broader search.
...
// Defined but never called in EE2 itself (it doesn't auto-search for most
// items by default - it waits for the user to review/adjust the checklist
// manually, see PriceCheckWidget.smartInitialSearch and CheckedItem.vue's
// `doSearch` gating). This app always searches immediately, so it needs an
// actual default-selection strategy - wired up in main/trade.ts.
export function enableGoodRolledFilters(filters: StatFilter[], abovePct: number) {
  for (const filter of filters) {
