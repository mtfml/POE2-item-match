...
import { STAT_BY_REF, CLIENT_STRINGS as _$, TRADE_TAG_TO_REF } from "../data";
...
export function createTradeRequest(
  ...
  const body: TradeRequest = {
    ...
    // Sort by most-recently-listed rather than cheapest - in a fast-moving
    // PoE2 league, recent listings are a much better price signal.
    sort: {
      indexed: "desc",
    },
  };
...
export async function requestTradeResultList(
  ...
  if (!data) {
    ...
    const response = await httpFetch(
      `https://${getTradeEndpoint()}/api/trade2/search/${leagueId}`,
      {
        method: "POST",
        headers: {
...
export async function requestResults(
  ...
  if (!data) {
    ...
    const response = await httpFetch(
      `https://${getTradeEndpoint()}/api/trade2/fetch/${resultIds.join(",")}?query=${queryId}`,
    );
