...
// EE2 supports multiple realms/trade sites based on user settings; this app
// only targets the main pc-ggg realm. Edit here if you play on Garena etc.
...
function _adjustRateLimits(
  ...
  const DEBUG = false;
  // EE2 exposes this as a user-tunable "apiLatencySeconds" setting to
  // correct for clock desync with GGG's rate-limit window; fixed at 0 here
  // since there's no settings UI.
  const DESYNC_FIX = 0;
